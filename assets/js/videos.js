...
(function() {
  ...
  const FALLBACK_ITEMS = [
    ...
    { title: "I Didn't Use Blender (Apology)", link: "https://www.youtube.com/watch?v=GyIhFGtcwgo", pubDate: new Date(Date.now() - 400 * 86400000).toISOString() }
  ];
  ...
  function renderCards(items) {
    return items.slice(0, 6).map(item => {
      ...
      <a href="${item.link}" class="video-card" target="_blank" rel="noopener">
        <div class="thumb">
          ${thumbSrc ? `<img src="${thumbSrc}" alt="${item.title}" loading="lazy">` : `<div class="thumb-placeholder">🎬</div>`}
  ...
  async function loadVideos() {
    ...
    if (cached) {
      ...
      if (Date.now() - data.ts < CACHE_TTL) {
        grid.innerHTML = renderCards(data.items);
        return;
      }
    }
    ...
    try {
      ...
      if (data.status === 'ok' && data.items?.length) {
        localStorage.setItem(CACHE_KEY, JSON.stringify({ ts: Date.now(), items: data.items }));
        grid.innerHTML = renderCards(data.items);
      } else {
        throw new Error();
      }
